...
local svers = "v539-1-6-2021-1242a-dark"
...
local function create_myglobals()
    ...
    if not global.cpf then
        global.cpf = 5
    end
    --Scan rate minumum
    if not global.chart_seconds then
        global.chart_seconds = 8
    end
    --Max scan chunks per frame
    --(attempt to keep scan rate, until this limit)
    if not global.cpf_max then
        global.cpf_max = 15
    end

    --Limit number of skips ( for blank chunks ) per frame
    if not global.skip_cap then
        global.skip_cap = 1000
    end
    ...
    function(event)
        if event and event.player_index and event.created_entity and event.stack then
            ...
            if player and player.valid then
                ...
                if created_entity and created_entity.force and created_entity.force.name and created_entity.force.name == "player" then
                    local pos = created_entity.position
                    if pos and global.chunk_index then
                        for _, chunk in ipairs(global.chunk_index) do
                            ...
                            if chunk.area.left_top.x < pos.x and chunk.area.left_top.y < pos.y then
                                if chunk.area.right_bottom.x > pos.x and chunk.area.right_bottom.y > pos.y then
                                    if not global.entity_list then
                                        global.entity_list = {}
                                    end

                                    --Insert item into object cache
                                    table.insert(global.entity_list, created_entity)
    ...
    function(event)
        if event and event.area and event.force and event.surface_index then
            ...
            if surf and event.force == game.forces["player"] then
                ...
                --Look for lights
                local light_found = surf.find_entities_filtered {area = event.area, force = "player", type = "lamp"}
                ...
                --No lights, look for fires
                if not found then
                    local fire_found = surf.find_entities_filtered {area = event.area, force = "player", type = "simple-entity-with-force"}
                    for _, fire in ipairs(fire_found) do
                ...
                --No lights or fires, look for radars
                if not found then
                    local radar_found = surf.find_entities_filtered {area = event.area, radius = global.radard, force = "player", type = "radar"}
                    for _, radar in ipairs(radar_found) do
        ...
        if event and event.entity and event.player_index then
            ...
            if player and player.character then
                if event.entity.name == "item-on-ground" and event.entity.stack and event.entity.stack.name == "wood" then
                    ...
                    local csurf = game.surfaces["nauvis"]
                    ...
                    local newpos = csurf.find_non_colliding_position("simple-entity-with-force", cpos, 99, 0.01, false)
                    local logobj = csurf.create_entity {name = "simple-entity-with-force", position = newpos, force = "player", render_player_index = 65535, player = player}
                    logobj.minable = false
                    logobj.rotatable = false
                    logobj.destructible = false

                    local firelight = rendering.draw_sprite {sprite = "file/fire/fire.png", target = logobj, render_layer = 122, surface = csurf, color = {1, 0.75, 0.15}, scale = 1, target_offset = {-0.25, -0.5}}
                    local fireimg = rendering.draw_light {sprite = "file/fire/light-medium.png", target = logobj, render_layer = 148, surface = csurf, color = {1, 0.75, 0.15}, scale = 5, target_offset = {-0.25, -0.5}}
...
script.on_nth_tick(
    ...
    function(event)
        ...
        if not global.player_dmg then
            ...
            surf.brightness_visual_weights = {1 / 0.85, 1 / 0.85, 1 / 0.85}
            surf.daytime = 0.5
        end
...
script.on_event(
    ...
    function(event)
        if not global.chunk_index then
            global.chunk_index = {}
        end
        if event and event.surface.name == "nauvis" then
            if event.surface.is_chunk_generated({event.position.x, event.position.y}) then
                table.insert(global.chunk_index, {area = event.area, x = event.position.x, y = event.position.y})
                global.chunk_count = global.chunk_count + 1
            end
        end
    end
)
...
script.on_nth_tick(
    1,
    function(event)
        --Process chunks, checking for areas without lights
        local csurf = game.surfaces["nauvis"]
        local cforce = game.forces["player"]
        local doskip = false
        local skipped = 0

        if not global.last_chunk_rebuild then
            global.last_chunk_rebuild = game.tick
            global.chunk_index = nil
        end

        --Rebuild cache every 30 minutes
        if global.chunk_pos == 1 and game.tick - global.last_chunk_rebuild > (30*60*60) then
            global.last_chunk_rebuild = game.tick
            global.chunk_index = nil
        end

        --If we dont have a chunk list, generate a new one
        if not global.chunk_index then
            message_all ("Rebuilding chart map data.")

            local chunk_list = csurf.get_chunks()
            global.chunk_pos = 0
            global.chunk_count = 0
            global.used_chunk_count = 0
            global.chunk_index = {}
            ...
            for chunk in chunk_list do
                --Is it a real chunk?
                if csurf.is_chunk_generated({chunk.x, chunk.y}) then
                    local item_count = csurf.count_entities_filtered {area = chunk.area, force = "player", limit = 1}

                    local light_found = csurf.find_entities_filtered {area = chunk.area, force = "player", type = {"lamp", "radar"}}

                    if light_found then
                        table.insert(global.light_list, light_found)
                    end
            ...
            global.chunk_count = count
            global.used_chunk_count = ucount

            global.cpf = math.ceil(ucount / global.chart_seconds / 60.0)

            --Cap to a maximum number per frame
            if global.cpf_max and global.cpf > global.cpf_max then
                global.cpf = global.cpf_max
            end
        else --We already cached chunks, lights and objects
            --Process global.cpf chunks this frame, skip empty (capped)

            local x = 0
            local cpf_real = 0
            cforce.cancel_charting(csurf)

            while x < global.cpf and skipped < global.skip_cap do
                cpf_real = cpf_real + 1
                if csurf and cforce and global.chunk_index then
                    local chunk = global.chunk_index[global.chunk_pos]
                    if chunk then
                        local found = false

                        --Don't bother if chunk is empty
                        if chunk.items and chunk.items > 0 then
                            if global.used_chunk_count then
                                global.used_chunk_count = global.used_chunk_count + 1
                            else
                                global.used_chunk_count = 0
                            end

                            --Don't chart if already charted
                            local charted = cforce.is_chunk_charted(csurf, {x = chunk.x, y = chunk.y})
                            --Don't rechart if we already have vision
                            --local visible = cforce.is_chunk_visible(csurf, {x = chunk.x, y = chunk.y})

                            --Find lights
                            local light_found = csurf.find_entities_filtered {area = chunk.area, force = "player", type = "lamp"}
                            for _, light in pairs(light_found) do
                                --If we find powered lights in uncharted map, chart it
                                if light.status ~= defines.entity_status.no_power then
                                    if not charted then
                                        cforce.chart(csurf, chunk.area)
                                    end
                                    found = true
                                    break
                                end
                            ...
                            --Find fires if no lights
                            if not found then
                                local fire_found = csurf.find_entities_filtered {area = chunk.area, force = "player", type = "simple-entity-with-force"}
                                for _, fire in pairs(fire_found) do
                                    if not fire.minable then
                                        --If we find powered lights in uncharted map, chart it
                                        if not charted then
                            ...
                            --Always look for radars, refresh them
                            local radar_found = csurf.find_entities_filtered {area = chunk.area, force = "player", type = "radar"}
                            for _, radar in pairs(radar_found) do
                                --If we find powered lights in uncharted map, chart it
                                if radar.status ~= defines.entity_status.no_power then
                                    --Don't refresh if we already have vision
                                    --if not visible then
                                        cforce.chart(csurf, chunk.area)
                                    --end
                                    found = true
                            ...
                            --Give us another chunk, this one is empty!
                            x = x - 1
                            skipped = skipped + 1
                        end

                        --No lights, fire or radars on chunk, unchart if charted
                        if not found then
                            cforce.unchart_chunk({x = chunk.x, y = chunk.y}, csurf)
                        end
                    end

                    if global.chunk_pos > global.chunk_count then
                        --Adjust scan speed
                        if global.used_chunk_count then
                            global.cpf = math.ceil(global.used_chunk_count / global.chart_seconds / 60.0)
                        ...
                        global.chunk_pos = global.chunk_pos + 1
                    end
                end
        ...
        --Process any unprocessed objected, detect light true/false
        if global.object_list then
            for _, light in pairs(global.light_list) do
                if light and light.valid then
                    local in_light = csurf.find_entities_filtered {position=light.position, force = "player"}

                else
                    --remove invalid object and warn
                end
            end
        end
