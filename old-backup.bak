...
script.on_load(
	function()
		...
		if (commands.commands.server_interface == nil) then
			...
			commands.add_command(
				"gspeed",
				"change game speed ( with auto walk speed adjustment )",
				function(param)
			...
			if (player.admin) then
				...
				if
					(global.actual_playtime and global.actual_playtime[player.index] and
						global.actual_playtime[player.index] > (15 * 60 * 60))
				 then
					if (player.permission_group ~= nil and player.permission_group.name == "Default") then
						if (global.trustedgroup.add_player(player) == true) then
							player.print(
								"(SERVER) You have now been playing long enough, that the new-user restrictions on your character have been lifted... Have fun, and be nice!"
							)
