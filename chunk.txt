...
function doit(target_surface, radius, keep_paving)
    ...
    if #playerForceNames > 1 then
        printAll({"DeleteEmptyChunks_text_force", table_to_csv(playerForceNames)})
    end
...
remote.add_interface(
    ...
    {
        getSurface = function()
            return settings.global["DeleteEmptyChunks_surface"].value
        end,
        getRadius = function()
            return settings.global["DeleteEmptyChunks_radius"].value
        end,
        getPaving = function()
            return settings.global["DeleteEmptyChunks_paving"].value
        end,
...
do ---- Init ----
    ...
    script.on_configuration_changed(
        function(data)
            for _, player in pairs(game.players) do
                if player and player.valid and player.admin then
                    if player.gui.left.DeleteEmptyChunks_button then
                        player.gui.left.DeleteEmptyChunks_button.destroy()
                    end
                    show_gui(player)
                end
            end
    ...
    script.on_event(
        ...
        function(event)
            ...
            local player = game.players[event.player_index]
            if not (player and player.valid and player.admin and gui and gui.valid) then
                return
            end
            local target_surface = settings.global["DeleteEmptyChunks_surface"].value
            local radius = settings.global["DeleteEmptyChunks_radius"].value
